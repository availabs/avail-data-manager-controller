...
async function main2() {
  await runInDamaContext(etl_context, async () => {
    ...
    const sel =
      "body > main > div > div.QueryFormContainer > div > div.left-panel > form > ol > li:nth-child(6) > div.DatasourceFieldsSelector > ul:nth-child(1) > li";
  ...
  await runInDamaContext(etl_context, async () => {
    ...
    for (let i = 0; i < 5; ++i) {
      ...
      if (!/checked$/.test(classNames)) {
        await page.click(
          `body > main > div > div.QueryFormContainer > div > div.left-panel > form > ol > li:nth-child(6) > div.DatasourceFieldsSelector > ul:nth-child(1) > li > ul > li:nth-child(${
            i + 1
    ...
    let aadtElClassNames = await (
      await aadtEl.getProperty("className")
    ).jsonValue();

    if (/checked$/.test(aadtElClassNames)) {
      await page.click(
        `body > main > div > div.QueryFormContainer > div > div.left-panel > form > ol > li:nth-child(6) > div.DatasourceFieldsSelector > ul:nth-child(1) > li > ul > li:nth-child(6) > span.CheckBoxGroup-node-content-wrapper.CheckBoxGroup-node-content-wrapper-normal`
      );

      aadtElClassNames = await (
        await aadtEl.getProperty("className")
      ).jsonValue();
...
async function main7() {
  const mdd = new MDD();
  await mdd.selectCanadianProvinceTmcsForYear(2023, "on");
}
